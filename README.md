...
[Security](#security) ·
[Capture privilege on the target](#capture-privilege-on-the-target) ·
...
the **Filters** library, which groups common expressions by protocol (Kerberos,
SMB, LDAP, DNS, database ports, TCP flag matching and so on). A running capture
reports how many packets it has taken so far. Captures can be renamed, and each
one remembers which server it came from even after that server is deleted.
...
list, with its own cheatsheet and examples; a filter tshark cannot parse is
reported with tshark's own message rather than shown as an empty list.
...
## Capture privilege on the target
